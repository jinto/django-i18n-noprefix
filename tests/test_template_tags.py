...
class TestTemplateTags:
    ...
    def test_language_selector_list_style(self, factory):
        ...
        assert 'i18n-noprefix-selector__item--active' in result or 'aria-current' in result
    ...
    def test_language_selector_with_next_url(self, factory):
        ...
        template = Template(
            "{% load i18n_noprefix %}"
            "{% language_selector next_url='/profile/' %}"
        )
        context = Context({'request': request})
        result = template.render(context)
...
class TestTemplateRendering:
    ...
    def test_accessibility_attributes(self, factory):
        ...
        template_dir = Path(__file__).parent.parent / 'django_i18n_noprefix' / 'templates' / 'i18n_noprefix'
        ...
        if template_dir.exists():
            ...
            if dropdown_template.exists():
                ...
                assert 'role' in content or '<select' in content  # select has implicit role

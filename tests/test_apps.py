...
class TestMiddlewareChecks:
    ...
    def test_conflicting_middleware(self):
        ...
        errors = check_middleware_configuration(None)
        assert any(
            isinstance(e, Error) and e.id == "django_i18n_noprefix.E001" for e in errors
        )
    ...
    def test_wrong_middleware_order_session(self):
        ...
        warnings = [
            e
            for e in errors
            if isinstance(e, Warning) and e.id == "django_i18n_noprefix.W002"
        ]
...
class TestLanguageChecks:
    ...
    def test_i18n_disabled(self):
        ...
        assert any(
            isinstance(e, Error) and e.id == "django_i18n_noprefix.E002" for e in errors
        )
    ...
    def test_empty_languages(self):
        ...
        errors = check_language_configuration(None)
        assert any(
            isinstance(e, Error) and e.id == "django_i18n_noprefix.E003" for e in errors
        )

    @override_settings(
        LANGUAGES=[("ko", "Korean"), ("en", "English")], LANGUAGE_CODE="fr"
    )
    def test_language_code_not_in_languages(self):
        ...
        warnings = [
            e
            for e in errors
            if isinstance(e, Warning) and e.id == "django_i18n_noprefix.W004"
        ]
        assert len(warnings) == 1
